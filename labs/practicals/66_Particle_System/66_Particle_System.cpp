...
bool load_content() {
	...
	glBufferData(GL_ARRAY_BUFFER, size(particles), &particles, GL_DYNAMIC_DRAW);
	glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, 0);

	// Fill space with blank data in buffer 2
	glBindTransformFeedback(GL_TRANSFORM_FEEDBACK, 1);
	glBindBuffer(GL_ARRAY_BUFFER, 1);
	glBufferData(GL_ARRAY_BUFFER, 1, memset(0,0,0), GL_DYNAMIC_DRAW);
	//TODO i think the issue is i don't fuckin know what fill space with blank data means
...
void render_feeback() {
	...
	{
		// unfortunately we can't save this stuff in the VAO. Because OGL.
